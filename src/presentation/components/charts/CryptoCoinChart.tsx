...
import type { CryptoCoinPerformanceData } from '@/application/hooks/useCryptoCoinPerformance';
import { formatCurrency } from '@/shared/utils/formatUtils';

interface CryptoCoinChartProps {
  data: CryptoCoinPerformanceData[];
  isLoading?: boolean;
  showWinRate?: boolean;
}

export const CryptoCoinChart = ({ data, isLoading, showWinRate = false }: CryptoCoinChartProps) => {
  if (isLoading) {
    return (
  ...
  const CustomTooltip = ({ active, payload }: TooltipContentProps<number, string>) => {
    if (active && payload && payload.length) {
      const entry = payload[0]?.payload as CryptoCoinPerformanceData | undefined;
      const value = payload[0]?.value;
      if (!entry || value === undefined) return null;
      return (
        <div className="bg-slate-900 border border-slate-700 rounded-lg p-3 shadow-xl">
          <p className="text-slate-400 text-sm mb-2 font-semibold">{entry.coin}</p>
          {showWinRate ? (
            <>
