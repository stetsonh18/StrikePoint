...
import type { ExpirationStatusData } from '@/application/hooks/useExpirationStatus';
import { formatCurrency } from '@/shared/utils/formatUtils';

interface ExpirationStatusChartProps {
...
export const ExpirationStatusChart = ({ data, isLoading, chartType = 'donut', metric = 'distribution' }: ExpirationStatusChartProps) => {
  if (isLoading) {
    return (
  ...
  const totalTrades = data.expired.totalTrades + data.closed.totalTrades;
  ...
  if (chartType === 'donut' && metric === 'distribution') {
    const pieData = [
      { name: 'Expired', value: data.expired.totalTrades, color: COLORS.expired },
      { name: 'Closed Early', value: data.closed.totalTrades, color: COLORS.closed },
    ];
  ...
  const barData = [
    {
      name: 'Expired',
      pl: data.expired.pl,
      winRate: data.expired.winRate,
      totalTrades: data.expired.totalTrades,
      winningTrades: data.expired.winningTrades,
      losingTrades: data.expired.losingTrades,
    },
    {
      name: 'Closed Early',
      pl: data.closed.pl,
      winRate: data.closed.winRate,
      totalTrades: data.closed.totalTrades,
      winningTrades: data.closed.winningTrades,
      losingTrades: data.closed.losingTrades,
    },
  ];
