  ...
  return (
    <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-900/50 dark:to-slate-800/30 backdrop-blur-sm rounded-2xl border border-slate-200 dark:border-slate-800/50 p-6 shadow-sm dark:shadow-none">
      ...
      <div className="flex gap-4 mb-4">
        ...
        <label className="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700/50 rounded-xl cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-800 transition-all">
          <input
            type="checkbox"
      ...
      <div className="overflow-x-auto">
        {isLoading ? (
          ...
          <div className="py-12 text-center">
            <AlertCircle className="w-12 h-12 text-slate-500 dark:text-slate-500 mx-auto mb-3" />
            <p className="text-slate-600 dark:text-slate-400">No contracts found</p>
            {searchQuery && (
              ...
              {filteredContracts.map((contract) => (
                ...
                >
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className="font-semibold text-slate-900 dark:text-slate-100">{contract.symbol}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className="text-sm text-slate-700 dark:text-slate-300">{contract.name}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className="text-sm text-slate-500 dark:text-slate-400">{contract.exchange || '-'}</span>
                  </td>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-center">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        ...
                        title="Edit contract"
                      >
                        <Edit2 size={16} className="text-slate-600 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400" />
                      </button>
                      <button
                      ...
                      >
                        <Power
                          size={16}
                          className={contract.is_active ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-500'}
                        />
                      </button>
                    </div>
                  </td>
