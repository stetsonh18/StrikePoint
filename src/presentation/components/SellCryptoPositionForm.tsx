...
interface SellCryptoPositionFormProps {
...
export const SellCryptoPositionForm: React.FC<SellCryptoPositionFormProps> = ({
  position,
  userId,
  onClose,
  onSuccess,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (quantityNum > maxQuantity) {
      setError(`Cannot sell more than ${maxQuantity} ${position.symbol}`);
      setIsSubmitting(false);
      return;
    }
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="sell-crypto-position-title"
      aria-describedby="sell-crypto-position-description"
    >
      <div
      ...
      >
        <div className="sticky top-0 bg-slate-900 border-b border-slate-700 p-6 flex items-center justify-between">
          <div>
            <h2 id="sell-crypto-position-title" className="text-2xl font-bold text-slate-100">Sell Crypto Position</h2>
            <p id="sell-crypto-position-description" className="text-sm text-slate-400 mt-1">
              {position.symbol} • {position.quantity.toFixed(8)} available
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="p-4 bg-slate-800/50 border border-slate-700 rounded-xl">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <div className="text-slate-400">Cryptocurrency</div>
              ...
              <div>
                ...
                <div className="text-slate-100 font-semibold">{position.quantity.toFixed(8)}</div>
              </div>
              <div>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-2">
              Quantity to Sell * (Max: {maxQuantity.toFixed(8)})
            </label>
            <input
              type="number"
              step="0.00000001"
            ...
            {quantityNum > 0 && (
              <div className="mt-2 text-xs text-slate-400">
                Selling {quantityNum.toFixed(8)} {position.symbol} ({((quantityNum / maxQuantity) * 100).toFixed(1)}% of position)
              </div>
            )}
          </div>
          ...
          <div className="flex gap-3 pt-4">
            ...
            >
              {isSubmitting ? 'Selling...' : 'Sell Crypto'}
            </button>
          </div>
        </form>
      </div>
