-- Regenerate all portfolio snapshots with corrected calculation
-- This fixes the portfolio_value formula to include realized P&L:
-- portfolio_value = net_cash_flow + total_realized_pl + total_market_value
...
BEGIN
  ...
  LOOP
    ...
    position_data AS (
      SELECT
        ...
        SUM(CASE WHEN (status = 'open' OR (status IN ('closed', 'expired', 'assigned', 'exercised') AND DATE(closed_at) > v_date))
             ...
             THEN COALESCE(unrealized_pl, 0) ELSE 0 END) AS total_unrealized_pl,
        SUM(CASE WHEN (status = 'open' OR (status IN ('closed', 'expired', 'assigned', 'exercised') AND DATE(closed_at) > v_date))
                  AND opened_at IS NOT NULL
                  AND DATE(opened_at) <= v_date
    ...
    RAISE NOTICE 'Generated snapshot for %', v_date;
  END LOOP;
